...
## `generate_ccf.py` の説明
### 概要
このプログラムは、RGB値と色名を含むデータからカラーチャートファイル（CCF）を生成します。RGB値は8ビットから16ビットに変換され、色名は ASCII または GB18030 文字コードでエンコードされます。
...
- **`encode_label` メソッド**: 色名をエンコードし、英数字と記号はASCII、中国語や日本語などは `EUC-CN/EUC-JP`が含まれている`GB18030` でエンコードします。
...
- **`decode_label` メソッド**: 色名をデコードし、英数字はASCII、中国語や日本語などは `EUC-CN/EUC-JP`が含まれている`GB18030` 文字コードでデコードします。
...
## 注意事項
1. ラベルのエンコードは、英数字と記号はASCII、中国語や日本語などは `EUC-CN/EUC-JP`が含まれている`GB18030` で行います。ラベルが15バイトを超える場合は切り捨てられます。
...
- **ラベル**: 16バイト　ラベルの長さの直後には、ラベルのうち、英数字、記号は`ASCII`形式で、日本語、中国語は`EUC-CN/EUC-JP`が含まれている`GB18030`でエンコードされて続きます。
